...
Status: FAST-LIO2 build and two GEODE development replays completed on 23 September 2026. Their execution manifests and pose streams are retained under ignored `generated/runs/`. Both replays produced poses, but neither provided a trustworthy motion estimate for the selected exit. The offline T07 evaluator has since been repaired; the full experiment suite has 28 passing tests, including four T08 source-math fixtures. Existing GEODE real-data rows explicitly mark formal errors unavailable because its reference body is unverified. A completed software run is not an accuracy result.

**New replacement-data trial:** [Hilti-Oxford Exp18](../data/HILTI_EXP18_REPLAY.md) has since been inspected and replayed for 55 seconds using the [Hesai adapter](src/hesai_time_adapter.py) and [exploratory configuration](../configs/fastlio_hilti_exp18_exploratory.yaml). Its scene exit was fixed from LiDAR alone, and the [development T07 pilot](../evidence/HILTI_EXP18_T07_PILOT.md) now reports actual 1 s/3 s local errors with unavailable counts. This is **not** a recovery result: matched post-exit reference coverage is too sparse and not independent of LiDAR map registration. The [development-only reference metadata](../data/hilti_exp18_reference_metadata.json) records the frame/clock choice and gaps. The GEODE record below is preserved as the earlier attempt, not replaced by the new trial.

**For future work, start with the [current handoff](../execution/CURRENT_HANDOFF.md).** The GEODE setup and commands below are historical reproduction instructions, not the next experiment to run.
...
The offline evaluator is `src/trajectory_eval.py`. Its standard-library test runner is `python3 -m unittest discover -s research_paper/experiments/tests -v` from the repository root (the Hilti structure check additionally needs the compatible isolated NumPy/SciPy environment recorded in the [event audit](../data/HILTI_EXP18_EVENT.md)). The CLI requires `--event-id` and either `--entry-start-ns` for a real pre-entry anchor or `--alignment-target-ns` for a separately named anchor. The original GEODE example uses `--event-id UT01_TUNNEL2 --entry-start-ns 1693022176719765425`. Without a GEODE body metadata file, all formal GEODE errors remain unavailable. The exact Hilti development command, input/output hashes and availability results are in the [T07 pilot](../evidence/HILTI_EXP18_T07_PILOT.md). The CLI verifies the Hilti reference file hash and rejects its metadata without `--development-only`. This does not certify independent ground truth.
